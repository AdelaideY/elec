...
require(['jquery', 'http'], function ($, http) {
    $(function () {
        $('header').load('./header.html', function () {$.post('./api/products.php').then(function (res) {
            ...
            $('.list_good_addcart').click(function () {
                ...
                data.forEach(function (item) {
                    if(item.name ==  $(this).parent().siblings('.list_good_des').find('b').text()){
                        ...
                        $.post('../src/api/details.php',{id:item.id,num: item.num}).then(function(res){
